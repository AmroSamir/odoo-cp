      ...
      <div className="bg-[#0f1b2d] border border-[#1e3a52] rounded-md w-full max-w-4xl h-[80vh] flex flex-col">
        <div className="flex items-center justify-between px-4 py-2 border-b border-[#1e3a52]">
          <div className="flex items-center gap-3">
            ...
            <span className={`text-[11px] font-mono ${isConnected ? 'text-green-400' : error ? 'text-red-400' : 'text-[#64748b]'}`}>{isConnected ? 'connected' : error ? 'error' : 'connecting'}</span>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={clear} className="text-[12px] text-[#64748b] hover:text-[#e2e8f0] px-2 py-1 transition-colors duration-150">Clear</button>
            <button onClick={handleCopy} className="text-[12px] text-[#64748b] hover:text-[#e2e8f0] px-2 py-1 transition-colors duration-150">Copy</button>
            {onClose && <button onClick={onClose} className="text-[#64748b] hover:text-[#e2e8f0] text-[16px] ml-1 px-1">x</button>}
          </div>
        </div>
