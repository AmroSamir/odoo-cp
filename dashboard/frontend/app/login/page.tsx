...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-arctic-bg">
      <div className="w-full max-w-sm">
        <div className="mb-8"><img src="/logo.png" alt="Logo" className="h-8 w-auto mx-auto mb-6" /></div>
        <form onSubmit={handleSubmit} className="bg-white border border-arctic-border rounded-md p-6 space-y-4 shadow-sm">
          <div>
            <label className="block text-[13px] text-[#4a7a8a] mb-1">Password</label>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter admin password" required autoFocus
              className="w-full bg-arctic-bg border border-arctic-border rounded-md px-3 py-2.5 text-[14px] text-[#0c4a6e] placeholder-[#94b3c2]" />
          </div>
