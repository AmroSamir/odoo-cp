      ...
      <div className="flex-1 ml-[240px]"><TopBar />
        <main className="pt-[48px] p-6">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-[16px] font-medium text-[#1e293b]">Git</h2>
          ...
          {loading && !status && <p className="text-[#64748b] text-[13px]">Loading...</p>}
          {status && (
            <div className="space-y-4">
              <div className="bg-page-surface border border-page-border rounded-md p-4">
                <div className="flex items-center gap-3 mb-3">
                  <span className="font-mono text-[14px] text-[#1e293b]">{status.branch}</span>
                  {status.isDirty && <span className="text-[11px] font-mono text-amber-500">dirty</span>}
                  {status.ahead > 0 && <span className="text-[11px] font-mono text-green-600">+{status.ahead} ahead</span>}
                  {status.behind > 0 && <span className="text-[11px] font-mono text-amber-500">-{status.behind} behind</span>}
                </div>
                <div className="flex gap-6 text-[12px]">
                  <div><span className="text-[#64748b]">Staged</span> <span className="text-[#1e293b] font-mono ml-1">{status.staged}</span></div>
                  <div><span className="text-[#64748b]">Modified</span> <span className="text-[#1e293b] font-mono ml-1">{status.modified}</span></div>
                </div>
              </div>
              <div>
                <h3 className="text-[13px] text-[#475569] mb-2">Commits</h3>
                  ...
                  {status.recentCommits.map((c, i) => (
                    <div key={c.hash} className="bg-page-surface border border-page-border rounded-md px-4 py-2.5">
                      <div className="flex items-start gap-3">
                        <span className="font-mono text-[10px] text-[#94a3b8] mt-0.5 flex-shrink-0">{c.hash}</span>
                        <div className="flex-1 min-w-0">
                          <p className="text-[13px] text-[#1e293b] truncate">{c.message}</p>
                          <p className="text-[11px] text-[#94a3b8] font-mono mt-0.5">{c.author} · {new Date(c.date).toLocaleString()}</p>
                        </div>
